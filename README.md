...
## 🚀 Hướng dẫn chi tiết Quy trình Cài đặt & Chạy ứng dụng

Sau khi đã hoàn tất cài đặt các công cụ tiền đề ở mục trên (Node.js, Python, FFmpeg), bạn hãy thực hiện theo đúng các bước sau để tải mã nguồn và khởi chạy dự án:

### Bước 1: Tải mã nguồn dự án (Clone Repository)
Hãy mở cửa sổ Terminal (PowerShell hoặc CMD) trên máy tính của bạn và chạy chuỗi lệnh sau để tải dự án về từ GitHub và di chuyển vào thư mục dự án:
```bash
# Tải dự án từ GitHub
git clone https://github.com/trakhanh/WaveForm-Edit-Studio.git

# Di chuyển vào thư mục dự án vừa tải về
cd WaveForm-Edit-Studio
```

---

### Bước 2: Cài đặt Thư viện Frontend (Node.js & Next.js)
Để cài đặt các gói thư viện giao diện, quản lý trạng thái và các icons, hãy chạy lệnh sau:
```bash
npm install
```
*(Tiến trình cài đặt có thể mất từ 1 - 2 phút tùy thuộc vào tốc độ mạng của bạn).*

---

### Bước 3: Cài đặt Thư viện Backend (Python Video Worker)
Ứng dụng sử dụng một tiến trình Python Worker chạy ngầm để thực hiện các phép xử lý nặng như tính toán tần số dải âm thanh, cắt bo viền ảnh chân dung và gọi FFmpeg ghép phụ đề. Hãy cài đặt các thư viện Python cần thiết bằng lệnh:
```bash
pip install -r worker-python/requirements.txt
```
> [!NOTE]
> * **openai-whisper:** Thư viện AI xử lý chuyển đổi giọng đọc thành phụ đề có dải thời gian chính xác.
> * **torch & torchaudio:** Công nghệ tăng tốc phần cứng của Facebook AI Research giúp Whisper xử lý âm thanh nhanh gấp 10-20 lần.
> * **pillow:** Thư viện xử lý hình ảnh nâng cao để tạo mặt nạ bo viền tròn/lục giác cho các lớp ảnh phủ.
> * **pydantic & fastapi-schema:** Dùng để kiểm định cấu trúc dữ liệu truyền tin giữa Node.js và Python Worker.

---

### Bước 4: Khởi chạy Máy chủ cục bộ (Development Server)
Khởi động máy chủ Next.js Dev Server cục bộ bằng lệnh sau:
```bash
npm run dev
```
Sau khi khởi chạy thành công, hãy mở trình duyệt web của bạn và truy cập vào đường dẫn:
👉 **`http://localhost:3000`**

Giao diện làm việc chuyên nghiệp, sang trọng của **WaveForm Edit Studio** đã sẵn sàng hoạt động!
